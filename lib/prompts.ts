import type { ResearchPlan, Paper, Insight, GapAnalysis } from "./schemas";
...
export function planPrompts(topic: string) {
  return {
    system:
      JSON_RULES +
...
/** Step 4 — Insight Extraction Agent (batched over all papers) */
export function insightPrompts(topic: string, papers: Paper[]) {
  const corpus = papers
    .map(
      (p) =>
        `### Paper ${p.id}\nTitle: ${p.title}\nYear: ${p.year ?? "n/a"}\nAbstract: ${
...
/** Step 5 + 6 + 7 — Gap Discovery Agent (themes + gaps + opportunities) */
export function gapPrompts(
  topic: string,
  papers: Paper[],
  insights: Insight[]
) {
...
/** Step 8 — Literature Review Agent (streaming markdown) */
